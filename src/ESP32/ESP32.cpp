...
bool ESP32::recv_ap(nsapi_wifi_ap_t *ap)
{
    ...
    while (true) {
        ...
        if (idx_0 >= (int)(sizeof(keyword_0) - 1)) {
            ...
            ret = _parser.recv("(%d,\"%32[^\"]\",%hhd,\"%hhx:%hhx:%hhx:%hhx:%hhx:%hhx\",%hhu,%d,%d,%d,%d,%d,%d)", &sec, ap->ssid,
                               &work_rssi[0], &work_buf[0], &work_buf[1], &work_buf[2], &work_buf[3], &work_buf[4],
                               &work_buf[5], &ap->channel, &freq_offset, &freqcal_val, &pairwise_cipher, &group_cipher, &bgn, &wps);
            ap->rssi = work_rssi[0];
